    ...
    <Tabs v-model="tab_index" size="small" @on-click="handleTabClick">
      ...
      >
        <StaffPerformanceList
          v-if="item.name === '业绩与提成' && tab_index === '业绩与提成'"
        />
        <ProjectPerformanceList
          v-if="item.name === '项目绩效' && tab_index === '项目绩效'"
        />
        <PackagePerformanceList
          v-if="item.name === '套餐次卡绩效' && tab_index === '套餐次卡绩效'"
        />
        <ValueCardPerformanceList
          v-if="item.name === '储值卡绩效' && tab_index === '储值卡绩效'"
        />
        <ConsultantPerformanceList
          v-if="item.name === '顾问绩效' && tab_index === '顾问绩效'"
        />
        <StaffTopList v-if="item.name === '排行榜' && tab_index === '排行榜'" />
      </TabPane>
    </Tabs>
  </div>
</template>
