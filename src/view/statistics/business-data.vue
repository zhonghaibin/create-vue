<template>
  <div class="box">
    ...
    <Tabs v-model="tab_index" type="card" @on-click="handleTabClick">
      ...
      >
        <DataOverview
          v-if="item.name === '数据总览' && tab_index === '数据总览'"
        />
        <BusinessAnalysis
          v-if="item.name === '经营分析' && tab_index === '经营分析'"
        />
        <ChannelAnalysis
          v-if="item.name === '渠道分析' && tab_index === '渠道分析'"
        />
        <VipAnalysis
          v-if="item.name === '会员分析' && tab_index === '会员分析'"
        />
        <ItemCardAnalysis
          v-if="item.name === '品项/卡分析' && tab_index === '品项/卡分析'"
        />
        <ContrastiveAnalysis
          v-if="item.name === '对比分析' && tab_index === '对比分析'"
        />
        <FinancialAnalysis
          v-if="item.name === '财务分析' && tab_index === '财务分析'"
        />
      </TabPane>
    </Tabs>
  </div>
